	...
	$(function() {
	  ...
	  submit.click(function(e) {
		...
		if (!isValid) {
		  ...
		  var submit_action = 'success_message';

		  if( $('.fl-node-<?php echo $id; ?> .fl-success-url').length ) {
		  	submit_action = 'redirect'
		  	var success_url = $('.fl-node-<?php echo $id; ?> .fl-success-url').val();
		  } else if( $('.fl-node-<?php echo $id; ?> .fl-success-none').length ) {
		  	submit_action = 'none'
		  }
		  ...
		  $.post(ajaxurl, {
			...
			if (response === '1') {
			  ...
			  if(submit_action === 'success_message') {       
			  	$('.fl-contact-form').hide();
			  	$('.fl-node-<?php echo $id; ?> .fl-success-msg').fadeIn();
			  } else if(submit_action === 'none') {
			  	$('.fl-node-<?php echo $id; ?> .fl-success-none').fadeIn();
			  } else {
