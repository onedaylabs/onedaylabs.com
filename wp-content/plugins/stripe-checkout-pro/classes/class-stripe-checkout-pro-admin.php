...
if ( ! class_exists( 'Stripe_Checkout_Pro_Admin' ) ) {
	class Stripe_Checkout_Pro_Admin extends Stripe_Checkout_Admin {
		...
		private function __construct() {
			...
			add_filter( 'plugin_action_links_' . plugin_basename( SC_DIR_PATH_PRO . 'stripe-checkout-pro.php' ), array( $this, 'settings_link' ) );
		...
		public function pro_settings_tab_default() {
			require_once( SC_DIR_PATH_PRO . 'views/admin-main-tab-default-pro.php' );
		}
		...
		public function set_admin_tabs( $tabs ) {
			...
			$tabs['license-keys'] = __( 'Licenses', 'sc' );
		...
		public function set_admin_tab_content() {
			...
			$sc_options->load_template( SC_DIR_PATH_PRO . 'views/admin-main-tab-license-keys.php' );
		}
